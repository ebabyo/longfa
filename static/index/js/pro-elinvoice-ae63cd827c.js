require(['KUYU.Service', 'KUYU.SlideBarLogin','KUYU.Header', 'KUYU.navHeader','juicer'], function() {
	var $http = KUYU.Service;
	var aa = KUYU.SlideBarLogin;
	var slidBarLogin = KUYU.SlideBarLogin;
	var invoice = {};
    var navHeader = KUYU.navHeader;
		...
		$http.post({
			...
			success: function(res) {
				...
				if ( res.data && res.data.length > 0 ) {
					...
					$("#electron_titleContent").attr("uuid",res.data[0].uuid).attr('hasNo',1);
                  ...
                  var person = $('.add-box .active .person').html();
                  $("#electron_titleContent").val(person).attr('hasNo',2);
                }
