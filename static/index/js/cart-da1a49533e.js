 ...
 * */
require([ 'KUYU.plugins.alert','juicer','KUYU.Store','KUYU.Header','KUYU.Service', 'KUYU.Binder','KUYU.navHeader'],function(){
    ...
    var shoppingFunc = {
        ...
        renderList: function (data, isLogin) {
            ...
            // if((!data || data == null)&&$Store.get('_ihome_token_')) {
            //     $(".Cart").hide();
            //     $(".box.gray-back ").hide();
            //     $(".buy-box").show();
            //     $(".buy-box2").hide();
            ...
            //     var login = $(".cart_login");
            //     login.attr("href",'http://user.tcl.com/proxy/login?from='+path);
            ...
            var funcPros = function(_pro, index){
                ...
                if( (_pro.onSell && !_pro.suitMain) || (_pro.suitMain && _pro.suitOnSell) ) {  //$.inArray(_pro.productId, arr_unmarketable) == -1
                    ...
                    if(_pro.promotionUuid !== null ||(_pro.promotionSubscribeOrReserveVo&&_pro.promotionSubscribeOrReserveVo.type)){
                        html += ' isActivity'
                    }
                ...
                if(_pro.suitMain && _pro.suitMain != null){
                    ...
                    html = html
                     ...
                     if(_pro.promotionUuid !== null){
                         html = html
                         +'<a class="active-tip" href="/pages/productDetail/productDetail.html?uuid='
                         + _pro.productId+'">秒杀进行中，点击这里下单 &gt; </a>';
                     }

                     if(_pro.promotionSubscribeOrReserveVo&&_pro.promotionSubscribeOrReserveVo.type){
                         html = html
                         +'<a class="active-tip" href="/pages/productDetail/productDetail.html?uuid='
                         + _pro.productId+'">预售进行中，点击这里下单 &gt; </a>';
                     }
                     html = html
                        + '</span>'
                        + '<span class="right">'
                        ...
                        if(_pro.couponList && _pro.couponList.length>0){
                            ...
                            $.each(_pro.couponList,function(i,item){
                                html = html
                                    ...
                                    + ' >'
                                    + '<span>满'
                                    + _pro.couponList[i].couCondition+'减'+_pro.couponList[i].denomination
                                    + '</span>'
                ...
                if(_pro.suitUuid ){
                    ...
                    html += parseFloat(_pro.singleMarketPrice ).toFixed(2);
                }
                html = html
                    + '元</span>'
                    + '</dd>'
                ...
                html = html
                    + '元</span><span class="j-price through fl total-price-old" style="color:gray;">'+( (_pro.marketFinalPrice && (parseFloat(_pro.marketFinalPrice) > parseFloat(_pro.totalPrice)))  ? parseFloat(_pro.marketFinalPrice).toFixed(2)+'元':"")+'</span>'
                    + '</dd>'
                ...
                if(_pro.suitMain && _pro.suitMain !=null) {
                    ...
                    html = html
                        ...
                        + '</span>元</dd><dd class="shop-item num-wid j-select " productId="'
                        + _pro.productId
                        + '" skuNo="';
                    if(_pro.attrIds!=null){
                        html += $.trim(_pro.attrIds);
            ...
            for(var i=0;i<data.carts.length;i++){
                if(data.carts[i].onSell==false||data.carts[i].stockNo==0){
                    unSellNum++;
                }
                if(unSellNum==data.carts.length){
                    $('.buy').addClass('disabled').attr("disabled" , true);
                }
            }
            // if(data.carts.length==1&&(data.carts[0].onSell==false||data.carts[0].stockNo==0)){
            //     $('.buy').addClass('disabled').attr("disabled" , true);
            // };
            //计算价格调用
        ...
        submit: function (res) {
            $("#buy").on("click", function () {
                ...
                var proid = null;
                $('.shop-item .active').each(function(i,o){
                    if($(this).hasClass('isActivity')){
                        isActivity = true;
                ...
                if(isActivity){
                    Msg.Confirm(
                        "温馨提示",
                        "您所购买的商品当前有秒杀或预售活动，商品更优惠哦~确认按原价下单？",
                        function(){
                            window.location.href = '/pages/productDetail/productDetail.html?uuid='+proid;
                        },
                        function(){//继续原价结算
                            if(res) {
        ...
        totalPrice: function (data) {
            ...
            var tpl =' <span class="pay-text pay-text-count">共'+data.cartProductCount+'件商品 已选择'+data.cartProductCount_sel+'件</span>'
                +' <span class="pay-text">合计（不含运费）：</span> '
                +'      <span class="pay-text red">'
                +'      <em class="font30 j-total">'+data.cartsTotalMount+'</em>元'
                +'      <s style="color:gray">'+(parseFloat(data.productTotalMount) > parseFloat(data.cartsTotalMount) ? parseFloat(data.productTotalMount).toFixed(2) +"元" : "" )+'</s>'
                +'</span>';
            subtotal.html(tpl);
        },
        //悬浮
    ...
    * */
                ...
                if(data.code ==1 ) {
                    ...
                    $.each(favDom, function (i, o) {
                        ...
                        if($.inArray(Id,favArr) > -1){
                            $('.star.j-star[productUuid="' + Id +'"]').addClass('active');
                        }
                    });
                ...
                success:function(data){
                   ...
                   //     unmask();
                        ...
                        NTKF_PARAM = {
                            ...
                            settingid: "kf_9428_1477382778975",   //接待组ID，为固定值，必填
    ...
    try{
        ...
        (function(d) {
            ...
            s.src = (d.location.protocol == 'https:' ? 'https:' : 'http:') + '//tc.solocpm.com/Visibility/Rtgdir2-min.js';
