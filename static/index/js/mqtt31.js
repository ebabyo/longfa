 ...
 */
if (typeof nTalk === "undefined") {
    nTalk = {}
}
nTalk.MQTT = function (global) {
    var version = "@VERSION@";
        ...
        for (key in obj) {
            if (obj.hasOwnProperty(key)) {
                if (keys.hasOwnProperty(key)) {
                    ...
                    for (k in keys) if (keys.hasOwnProperty(k)) errorStr = errorStr + " " + k;
    ...
    WireMessage.prototype.encode = function () {
        ...
        switch (this.type) {
            ...
            case MESSAGE_TYPE.SUBSCRIBE:
                ...
                break;
            case MESSAGE_TYPE.UNSUBSCRIBE:
                for (i = 0; i < this.topics.length; i++) pos = writeString(this.topics[i], topicStrLength[i], byteStream, pos);
                break;
            default:
        }
        return buffer
    ...
    function decodeMessage(input, pos) {
        ...
        switch (type) {
            ...
            case MESSAGE_TYPE.PUBLISH:
                ...
                var message = new nTalk.MQTT.Message(input.subarray(pos, endPos));
    ...
    var ClientImpl = function (uri, host, port, path, clientId) {
        ...
        this._trace("nTalk.MQTT.Client", uri, host, port, path, clientId);
        this.host = host;
        this.port = port;
        this.path = path;
        this.uri = uri;
    ...
    ClientImpl.prototype.hostIndex = 0;
    ClientImpl.prototype.onConnectionLost = function () {
    };
    ClientImpl.prototype.onMessageDelivered = function () {
    };
    ClientImpl.prototype.onMessageArrived = function () {
    };
    ClientImpl.prototype.traceFunction = function () {
    };
    ...
    ClientImpl.prototype.clear = function () {
        for (var key in localStorage) if (key.indexOf("Sent:" + this._localKey) === 0 || key.indexOf("Received:" + this._localKey) === 0) localStorage.removeItem(key)
    };
    ClientImpl.prototype.restore = function (key) {
        ...
        switch (storedMessage.type) {
            case MESSAGE_TYPE.PUBLISH:
                ...
                var payloadMessage = new nTalk.MQTT.Message(byteStream);
    ...
    ClientImpl.prototype._handleMessage = function (wireMessage) {
        var key, sentMessage, receivedMessage, pubRelMessage;
            ...
            switch (wireMessage.type) {
                case MESSAGE_TYPE.CONNACK:
                    ...
                    for (var i = 0, len = sequencedMessages.length; i < len; i++) {
                        sentMessage = sequencedMessages[i];
                        if (sentMessage.type == MESSAGE_TYPE.PUBLISH && sentMessage.pubRecReceived) {
                            pubRelMessage = new WireMessage(MESSAGE_TYPE.PUBREL, {messageIdentifier: sentMessage.messageIdentifier});
                ...
                case MESSAGE_TYPE.PUBREC:
                    ...
                    if (sentMessage) {
                        ...
                        pubRelMessage = new WireMessage(MESSAGE_TYPE.PUBREL, {messageIdentifier: wireMessage.messageIdentifier});
                ...
                case MESSAGE_TYPE.SUBACK:
                    ...
                    if (sentMessage) {
                        ...
                        if (wireMessage.returnCode.indexOf && wireMessage.returnCode.indexOf(128) !== -1) {
    ...
    ClientImpl.prototype._trace = function () {
        ...
        if (this.traceFunction) {
            for (i in args) {
                if (typeof args[i] !== "undefined") args[i] = JSON.stringify(args[i])
            }
        ...
        if (this._traceBuffer !== null) {
            for (i = 0, max = args.length; i < max; i++) {
                ...
                if (i === 0) this._traceBuffer.push(args[i]); else if (typeof args[i] === "undefined") this._traceBuffer.push(args[i]); else this._traceBuffer.push("  " + JSON.stringify(args[i]))
            }
        }
    };
    ...
    var Message = function (newPayload) {
        ...
        this.payloadString = typeof payload === "string" ? payload : parseUTF8(payload, 0, payload.length);
        var buffer, byteStream;
        if (typeof payload === "string") {
            buffer = new ArrayBuffer(UTF8Length(payload));
            byteStream = new Uint8Array(buffer);
            stringToUTF8(payload, byteStream, 0);
            this.payloadBytes = byteStream
        } else {
            this.payloadBytes = payload
        }
    };
    Message.prototype = {
        setDestinationName: function (newDestinationName) {
            if (typeof newDestinationName === "string") this.destinationName = newDestinationName; else throw new Error(format(ERROR.INVALID_ARGUMENT, [newDestinationName, "newDestinationName"]))
        }, setQos: function (newQos) {
            if (newQos === 0 || newQos === 1 || newQos === 2) qos = newQos; else throw new Error("Invalid argument:" + newQos)
        }, setRetained: function (newRetained) {
            if (typeof newRetained === "boolean") this.retained = newRetained; else throw new Error(format(ERROR.INVALID_ARGUMENT, [newRetained, "newRetained"]))
        }, setDuplicate: function (newDuplicate) {
